            ...
            <nav className="flex items-center max-w-screen-xl mx-auto px-6 py-3">
                ...
                <div className="hidden md:flex lg:flex space-x-8">
                    <ul className="flex items-center space-x-4">
                        {menu.map(item => (
                            <li key={item.id}>
                                <NavLink  to={item.to} className="text-gray-600 text-lg poppins" activeClassName="border-b-4 border-blue-600 text-blue-700">{item.text}</NavLink>
                            </li>
                        ))}
                    </ul>
                        ...
                        <ul className="mb-2">
                            {menu.map(item => (
                                <li key={item.id} className="mb-3">
                                    <NavLink  key={item.id} to={item.to} className="text-gray-600 poppins text-lg text-center py-2 px-3 w-full hover:bg-gray-200 transition duration-300 cursor-default block rounded-lg" activeClassName="border-l-4 border-blue-700 bg-gray-200">
                                    {item.text}
                                    </NavLink>
                                </li>
                            ))}
